...
def remove_background(image_path, hue_margin=30, sat_margin=100, val_margin=255):
    ...
    _, labels, centers = cv2.kmeans(np.float32(flattened_image), 1, None, criteria, 10, cv2.KMEANS_RANDOM_CENTERS)
    most_common_color = np.uint8(centers[0])
    # Define the range for background color with a margin of error
    lower_bound = np.array(
        [most_common_color[0] - hue_margin, most_common_color[1] - sat_margin, most_common_color[2] - val_margin])
...
def blob_detection(gray_image):
    ...
    img_key_points = cv2.drawKeypoints(gray_image, keypoints, np.array([]), (0, 0, 255),
